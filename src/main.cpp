...
int main(int argc, char** argv) {
    ...
    if (!loadLandPolygons(cfg.land_geojson, all_land)) {
        std::cerr << "Dataset yok. Once su scripti calistirin: scripts/download_coastline_data.sh\n";
        return 1;
    }
    ...
    if (cfg.mesh_bounds_override) {
        ...
        if (min_lat >= max_lat || min_lon >= max_lon) {
            std::cerr << "Mesh sinirlari gecersiz. mesh.min/max degerlerini kontrol edin.\n";
            return 1;
        }
    ...
    if (!pointInsideBounds(start_point.geo) || !pointInsideBounds(goal_point.geo)) {
        std::cerr << "Start/goal mesh kapsami disinda. route/start-goal veya mesh.min/max sinirlarini guncelleyin.\n";
        return 1;
    }
    ...
    if (cfg.use_cache_enabled) {
        if (cfg.use_cache_file.empty()) {
            std::cerr << "Cache yukleme aktif ama --use-cache <file> verilmedi.\n";
            return 1;
        }
        ...
        if (!isPlannerCacheCompatible(cache,
                                      cfg,
                                      min_lat,
                                      max_lat,
                                      min_lon,
                                      max_lon,
                                      coastline_clearance_km * 1000.0)) {
            return 1;
        }
        ...
        if (!cfg.build_cache_file.empty()) {
            ...
            cache.mesh_dataset_path = cfg.land_geojson;
    ...
    if (start_ranked.empty() || goal_ranked.empty()) {
        std::cout << "Mesh uzerinde kullanilabilir baslangic/varis vertex'i bulunamadi.\n";
        return 1;
    }
    ...
    if (mesh_start_vertex < 0 || goal_vertex < 0) {
        ...
        printRoute(RouteResult{});
        std::filesystem::create_directories("output");
        writeSvgPlot(cfg.svg_file, vertices, triangles, {}, {}, land, tss, bathymetry);
        std::cout << "Start (sea): (" << start_point.geo.lat_deg << ", " << start_point.geo.lon_deg << ")\n";
        std::cout << "Goal (sea): (" << goal_point.geo.lat_deg << ", " << goal_point.geo.lon_deg << ")\n";
    ...
    printRoute(route);
    if (route.found && route.polyline.size() >= 2) {
        int crossing_segments = 0;
        for (size_t i = 1; i < route.polyline.size(); ++i) {
            if (!edgeNavigable(route.polyline[i - 1],
                               route.polyline[i],
                               land,
                               land_index,
                               coastline_clearance_km,
                               ...
                               shallow_index)) {
                crossing_segments += 1;
                std::cout << "UYARI: Segment " << i << " kara/sig alandan geciyor olabilir.\n";
            }
        }
        std::cout << "Kisit-kesen segment sayisi: " << crossing_segments << "\n";
    }

    std::filesystem::create_directories("output");
    writeSvgPlot(cfg.svg_file, vertices, triangles, route.triangle_sequence, route.polyline, land, tss, bathymetry);
    std::cout << "Start (sea): (" << start_point.geo.lat_deg << ", " << start_point.geo.lon_deg << ")\n";
    std::cout << "Goal (sea): (" << goal_point.geo.lat_deg << ", " << goal_point.geo.lon_deg << ")\n";
    std::cout << "Plot dosyasi: " << cfg.svg_file << "\n";

    return route.found ? 0 : 1;
}
